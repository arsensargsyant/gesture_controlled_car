...
void loop()
{
    ...
    if(radio.isChipConnected() && radio.available())    {
      ...
      int delta = 30;
      int xAxisValue = receiverData.xAxisValue;
      if (xAxisValue > 127 - delta && xAxisValue < 127 + delta) 
        xAxisValue = 127;
      int yAxisValue = receiverData.yAxisValue;
      if (yAxisValue > 127 - delta && yAxisValue < 127 + delta) 
        yAxisValue = 127;


      int mappedYValue = map(yAxisValue, 0, 254, -255, 255); 
      int mappedXValue = map(xAxisValue, 0, 254, 255, -255); 
